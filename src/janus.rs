...
const OPUS: AudioParameter = AudioParameter {
    encoder: "opusenc",
    encoding_name: "OPUS",
...
impl std::str::FromStr for AudioParameter {
    type Err = anyhow::Error;
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s {
            "opus" => Ok(OPUS),
            _ => Err(anyhow!(
                "Invalid audio parameter: {}. Only opus supported",
                s
            )),
        }
    }
...
impl JanusGateway {
    pub async fn new(pipeline: gst::Pipeline) -> Result<Self, anyhow::Error> {
        ...
        if let Some(codec) = args.video {
            ...
            let video_src_elem = video_bin
                .get_static_pad("src")
                .expect("Unable to get video src pad");
            let video_sink = webrtcbin
                .get_request_pad(format!("sink_{}", sink_index).as_str())
        ...
        if let Some(codec) = args.audio {
            ...
            let audio_src_elem = audio_bin
                .get_static_pad("src")
                .expect("Unable to get audio src pad");
            let audio_sink = webrtcbin
                .get_request_pad(format!("sink_{}", sink_index).as_str())
                .expect("Unable to request outgoing webrtcbin pad");

            if let Ok(webrtc_ghost_audio_src) =
                gst::GhostPad::with_target(Some("webrtc_audio_src"), &audio_src_elem)
            {
